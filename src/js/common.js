...
function setTimer(num) {
	...
	var mon = toDB(time.getMonth() + 1)//月
	var day = toDB(time.getDate())//日24小时
...
var cookie = {
	...
	remove: function (key) {//key 为id
		//cookie:设置时间失效，设置时间为过去的某个时间
		var now = new Date();
		now.setDate(now.getDate() + 1);
		cookie.set(key, '0', { expires: now, path: '/' });
	}
}
...
function ajax(method, url, data, fn) {
	...
	//告诉对象，要什么
	if (method.toLowerCase() == 'get') {//如果是get的方式，data接在url后面
		if (data) {
			//如果是get方法并有数据
			url = url + '?' + data;
		}
	...
	xhr.onreadystatechange = function () {
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				...
				alert('出错了，因为：' + xhr.status);//返回错误状态
			}
...
function tabq(cc, hh, xx, atv) {
	for (var i = 0; i < cc.length; i++) {
		...
		cc[i].onclick = function () {
			...
			this.className = atv;
			hh[this.index].style.display = 'block';
